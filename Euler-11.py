...
01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48'''
    ...
    for row in range(0, 20):
        for index in range(0, 20):
            ...
            # check vertical
            if row < 17:
                temp_numbers = [rows[row][index], rows[row+1][index], rows[row+2][index], rows[row+3][index]]
                winning_total = compare(winning_total, temp_numbers)

            # check diag down and right (same as up and left)
            if row < 17 and index < 17:
                temp_numbers = [rows[row][index], rows[row+1][index+1], rows[row+2][index+2], rows[row+3][index+3]]
                winning_total = compare(winning_total, temp_numbers)

            #check diag down and left ( same as up and right)
            if index > 2 and row < 17:
                temp_numbers = [rows[row][index], rows[row+1][index-1], rows[row+2][index-2], rows[row+3][index-3]]
                winning_total = compare(winning_total, temp_numbers)
